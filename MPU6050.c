...
void MPU6050_Get_Accel(MPU6050_ACCEL_t* Accel_Instance){
	...
	//CODE_FILL
	Accel_Instance->Ax_RAW = (uint16_t)((ACCEL_X_HIGH << 8) | ACCEL_X_LOW);
	Accel_Instance->Ay_RAW = (uint16_t)((ACCEL_Y_HIGH << 8) | ACCEL_Y_LOW);
	Accel_Instance->Az_RAW = (uint16_t)((ACCEL_Z_HIGH << 8) | ACCEL_Z_LOW);
...
void MPU6050_Get_Gyro(MPU6050_GYRO_t* Gyro_Instance){
	...
	//CODE_FILL
	Gyro_Instance->Gx_RAW = (uint16_t)((GYRO_X_HIGH << 8) | GYRO_X_LOW);
	Gyro_Instance->Gy_RAW = (uint16_t)((GYRO_Y_HIGH << 8) | GYRO_Y_LOW);
	Gyro_Instance->Gz_RAW = (uint16_t)((GYRO_Z_HIGH << 8) | GYRO_Z_LOW);
...
void MPU6050_Get_Angle(MPU6050_ACCEL_t* Accel_Instance, MPU6050_GYRO_t* Gyro_Instance, MPU6050_ANGLE_t* Angle_Instance){
	...
	/*CODE_FILL*/
	// Calculate Tilt Angle using processed Accelerometer and Gyroscope data and store it in the user angle struct
	...
	Angle_Instance->ArZ = atan2f(Accel_Instance->Ax, Accel_Instance->Ay) * RAD_TO_DEGREE_CONV;
